...
const App: React.FC = () => {
  ...
  // Combat loop
  useEffect(() => {
    if (!isAutoExploring || player.hp <= 0) return;

    const autoAttackInterval = setInterval(handleAttack, 1000);
    return () => clearInterval(autoAttackInterval);
  }, [isAutoExploring, player.hp]);
  ...
  const handleAttack = () => {
    ...
    let updatedPlayer = { ...player, hp: Math.max(newHp, 0) };
    ...
    if (defeated) {
      ...
      updatedPlayer = gainXP({ ...updatedPlayer, souls: updatedPlayer.souls + soulsGain }, xpGain);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      <Navigation 
        ...
        manaPercent={100}
        xpPercent={(player.xp / player.xpToNext) * 100}
        currentFloor={floor}
      />
      ...
      <div className="container mx-auto px-4 pb-8">
        ...
        {currentPage === 'Explore' && (
          <Explore
            ...
            onToggleAutoExplore={() => setIsAutoExploring(!isAutoExploring)}
          />
        )}
